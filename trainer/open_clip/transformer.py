...
class AttentionPETL(nn.Module):
    ...
    def __init__(
        ...
        self.head_dim = dim // num_heads
        self.scale = self.head_dim ** -0.5
        self.fused_attn = hasattr(nn.functional, 'scaled_dot_product_attention')
        ...
        if self.peft_config.lora_bottleneck > 0:
            self.lora = LoRA(dim, num_heads, peft_config)
        self.fact = fact
        ############# Added module end #############
    def forward(self, x: torch.Tensor, block_idx=None) -> torch.Tensor:
        B, N, C = x.shape
        ############# Added module #############
        # q, k, v = F.linear(x, self.in_proj_weight, self.in_proj_bias).chunk(3, dim=-1)
        ...
        # q = q.reshape(B, N * self.num_heads, -1).transpose(0, 1)
        # k = k.reshape(B, N * self.num_heads, -1).transpose(0, 1)
        # v = v.reshape(B, N * self.num_heads, -1).transpose(0, 1)
        ...
        if self.peft_config.vqt_num > 0:
            k = k[:, :, self.peft_config.vqt_num:]
            v = v[:, :, self.peft_config.vqt_num:]
        ############# Added module end #############
...
class CustomResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        # )
        with open(f"/home/tian.855/Camera_Trap_Search/trainer/open_clip/lora/dim{LoRA_dim}/config.json", "r") as file:
            cfg = json.load(file)
        cfg = Box(cfg)
        peft_config = cfg.petl_config
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        ...
        x = x + self.ls_1(self.ln_attn(self.attn(self.ln_1(x))))
        x = x + self.ls_2(self.mlp(self.ln_2(x)))
        return x
