...
def _load_weights_PETL(model: VisionTransformerPETL, checkpoint_path: str, prefix: str = ''):
    if checkpoint_path.endswith('.npz'):
        ...
        # _load_weights_pth(model, checkpoint_path, checkpoint_filter_fn_resize_pos_embed)
        _load_weights_pth(model, checkpoint_path, checkpoint_filter_fn)

def checkpoint_filter_fn_resize_pos_embed(state_dict, model):
    if 'pos_embed' in state_dict and hasattr(model, 'pos_embed'):
        posemb_pretrained = state_dict['pos_embed']
        posemb_model = model.pos_embed
        # Interpolate positional embedding if shape mismatches
        if posemb_pretrained.shape != posemb_model.shape:
            num_prefix_tokens = model.num_prefix_tokens
            state_dict['pos_embed'] = resample_abs_pos_embed(
                posemb_pretrained, posemb_model, num_prefix_tokens=num_prefix_tokens
            )
    return state_dict

def _load_weights_pth(model, checkpoint_path, filter_fn=checkpoint_filter_fn_resize_pos_embed):
...
def vit_base_patch14_dinov2_petl(pretrained: bool = False, **kwargs):
    ...
    """
    img_size = int(kwargs['params'].img_size)
    model_args = dict(patch_size=14, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, img_size=img_size)
    model = _create_vision_transformer_petl(
        'vit_base_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_base_patch16_dinov2_petl(pretrained: bool = False, **kwargs):
    """ ViT-B/14 for DINOv2
    change img_size to 224
    """
    img_size = int(kwargs['params'].img_size)
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, img_size=img_size)
    model = _create_vision_transformer_petl(
        'vit_base_patch16_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_large_patch14_dinov2_petl(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    change img_size to 224
    """
    img_size = int(kwargs['params'].img_size)
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=img_size)
    model = _create_vision_transformer_petl(
        'vit_large_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_large_patch16_siglip2_petl(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    change img_size to 224
    """
    img_size = int(kwargs['params'].img_size)
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=img_size)
    model = _create_vision_transformer_petl(
        'vit_large_patch16_siglip_512', pretrained=pretrained, **dict(model_args, **kwargs))
    return model

@register_model
def vit_base_patch16_siglip2_petl(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    change img_size to 224
    """
    img_size = int(kwargs['params'].img_size)
...
@register_model
def vit_so400m_patch16_siglip2_petl(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    change img_size to 224
    """
    img_size = int(kwargs['params'].img_size)
    model_args = dict(patch_size=16, embed_dim=1152, depth=32, num_heads=16, init_values=1e-5, img_size=img_size)
    model = _create_vision_transformer_petl(
...
@register_model
def vit_base_patch16_bioclip_224_petl(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ ViT-B/16 CLIP image tower
    """
    img_size = int(kwargs['params'].img_size)
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, pre_norm=True, norm_layer=nn.LayerNorm,
                      act_layer='quick_gelu')
    model = _create_vision_transformer_petl(
        'vit_base_patch16_bioclip_quickgelu_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
