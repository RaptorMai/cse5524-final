...
OUTPUT_DIR = 'logs'
MODEL_DIR = '/research/nfs_chao_209/snapshot/MODEL/'
RESULT_DIR = 'submissions'

def train(params, model, tune_parameters, train_loader, val_loader, test_loader):
    trainer = Trainer(model, tune_parameters, params)
    if os.path.isfile(os.path.join(params.model_dir, 'model.pt')):
        logger.info('load model from {}'.format(os.path.join(params.model_dir, 'model.pt')))
        trainer.load_weight(os.path.join(params.model_dir, 'model.pt'))
    if params.run_method == 'train_only':
        logger.info('Train on training set only')
        train_metrics, best_eval_metrics, eval_metrics = trainer.train_classifier(train_loader, val_loader, test_loader)
    elif params.run_method == 'trainEval':
        logger.info('Train on training set and validation set')
        val_loader.dataset.set_mode('train')
        train_loader.dataset.merge(val_loader.dataset)
        train_loader.sampler.reconstruct(train_loader.dataset)
        train_metrics, best_eval_metrics, eval_metrics = trainer.train_classifier(train_loader, val_loader, test_loader)
    else:
        raise NotImplementedError(f'run {params.run_method} not implemented')
    logger.info(
        f'train acc {train_metrics["top1"]}, best acc {best_eval_metrics["top1"]}, last acc {eval_metrics["top1"]}')
    return train_metrics, best_eval_metrics, eval_metrics, trainer.model

def test(params, model, tune_parameters, train_loader, val_loader, test_loader):
    trainer = Trainer(model, tune_parameters, params)
    if os.path.isfile(os.path.join(params.model_dir, 'model.pt')):
        logger.info('load model from {}'.format(os.path.join(params.model_dir, 'model.pt')))
        ...
        logger.info('no model.pt, show zero shot performance')
    logger.info('Test on test set only')

    dataset_name = 'fungi'
    data_name = f'{dataset_name}_{params.data_size}_{params.img_quality}_{params.img_size}'
    method = method_name(params)
    method = f'{method}_{params.dataAug}_{params.loss}'
    sub_path = os.path.join(RESULT_DIR, params.pretrained_weights, dataset_name, method, params.test_method)
    os.makedirs(sub_path, exist_ok=True)
    sub_path = os.path.join(sub_path, 'submisson.csv')
    train_loader.dataset.merge(val_loader.dataset)
    trainer.test_classifier(train_loader, test_loader, params.test_method, sub_path, params)
...
def basic_run(params):
    ...
    method = method_name(params)
    method = f'{method}_{params.dataAug}_{params.img_size}_{params.loss}'
    # method = f'{method}_{params.dataAug}_{params.loss}'
    start_time = datetime.now().strftime("%Y-%m-%d-%H:%M")
    output_dir = os.path.join(OUTPUT_DIR, params.pretrained_weights, dataset_name, method, run_method, start_time)
    model_dir = os.path.join(MODEL_DIR, params.pretrained_weights, dataset_name, method)
    os.makedirs(output_dir, exist_ok=True)
    os.makedirs(model_dir, exist_ok=True)
    log_path = setup_logging(output_dir, params.debug)
    ...
    if params.run_method != 'test':
        train(params, model, tune_parameters, train_loader, val_loader, test_loader)
    else:
        test(params, model, tune_parameters, train_loader, val_loader, test_loader)
